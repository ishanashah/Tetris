...
public class JTetris extends JComponent {
    ...
    //Karma
    //Stores the Pieces left to be generated before a new random stack is created.
    protected Stack<Piece> randomPieces = new Stack();
    ...
    JTetris() {
        ...
        registerKeyboardAction(new ActionListener() {
                ...
                "hold", KeyStroke.getKeyStroke(' '), WHEN_IN_FOCUSED_WINDOW);
     ...
     * set in startGame().
     */
    //Uses the Random_Generator algorithm defined in tetris.wikia.com
    public Piece pickNextPiece() {
        if(randomPieces.empty()){
            java.util.List<Piece> pieceList = Arrays.asList(PIECES);
            Collections.shuffle(pieceList);
            for(int i = 0; i < pieceList.size(); i++){
                randomPieces.push(pieceList.get(i));
            }
        }
        return randomPieces.pop();
    }
    ...
    public void paintComponent(Graphics g) {
        ...
        //g.drawRect(getWidth(), getHeight() - 4, 4, 4);
